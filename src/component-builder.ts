...
import { ComponentData, ColorData, TypographyData } from './main';
...
export class ComponentBuilder {
  ...
  private async applyPaintStyles(
    ...
    if (paints.length === 1 && paints[0].type === 'SOLID') {
      ...
      if (!style) {
        style = figma.getStyleByName(styleName) as PaintStyle;
      }
  ...
  private async createOrReuseTextStyle(
    ...
    if (!style) {
      style = figma.getStyleByName(styleName) as TextStyle;
    }
